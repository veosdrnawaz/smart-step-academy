      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8">
          {stats.map((stat, index) => (
            ...
            >
              <div className="bg-primary/10 w-10 h-10 md:w-12 md:h-12 flex items-center justify-center rounded-full text-primary mb-3 text-lg md:text-xl">
                <i className={`fa-solid ${stat.icon}`}></i>
              </div>
              <h4 className="text-2xl md:text-3xl font-bold text-white">{stat.value}</h4>
              <p className="text-muted text-xs md:text-sm uppercase tracking-wider font-medium">{stat.label}</p>
            </motion.div>
          ))}
        </div>
      </div>
